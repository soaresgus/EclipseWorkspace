...
public class StorageObject extends StorageBase {
		...
		while (!claz.equals(Object.class)) {
			...
			for (Field field : claz.getDeclaredFields()) {
				...
				try {
					...
					// Object fieldRestored = new
					// StorageList(getInfo().clone()).restore(fieldMapValue);
					// field.set(instance, fieldRestored);
					//
					// } else if (Extra.isMap(field.getType())) {
					// Object fieldRestored = new StorageMap(field, instance,
					// reference).restore(fieldMapValue);
					// field.set(instance, fieldRestored);
					//
					// } else {
	...
	public Object store(Object data) {
		...
		if (claz.isEnum()) {
			...
			// return new StorageEnum(getField(), getInstance(), getType(), isReference(),
			// isInline(), isIndentifiable())
			// .store(data);
		}
		if (claz.isArray()) {
			debug("<< ARRAY " + data);
		...
		try {
			...
			while (!claz.equals(Object.class)) {
				for (Field field : claz.getDeclaredFields()) {
					...
					// if (Extra.isList(field.getType())) {
					// fieldResult = new StorageList(Extra.getTypeKey(field.getGenericType()),
