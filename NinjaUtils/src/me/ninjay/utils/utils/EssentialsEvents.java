...
public class EssentialsEvents extends EventsManager {
	...
	public void onEnable(PluginEnableEvent e) {
		...
		for (Config config : Config.CONFIGS) {
			if (e.getPlugin().equals(Main.getInstance().getConfigs().getPlugin())) {
				config.reloadConfig();
			}
		}
	}
	...
	public void onPingServer(ServerListPingEvent e) {
		Integer amount = Main.getInstance().getConfigs().getInt("custom-motd-amount");
		if (amount > -1) {
			e.setMaxPlayers(amount);
		}

		if (Main.getInstance().getConfigs().getBoolean("custom-motd")) {
			StringBuilder builder = new StringBuilder();
			for (String line : Main.getInstance().getConfigs().getMessages("motd")) {
				builder.append(line + "\n");
			}
	...
	public void onQuit(PlayerQuitEvent e) {
		Player p = e.getPlayer();
		if (Main.getInstance().getConfigs().getBoolean("custom-quit-message"))
	...
	public void onJoin(PlayerJoinEvent e) {
		Player p = e.getPlayer();
		if (Main.getInstance().getConfigs().getBoolean("save-players")) {
			InfoGenerator.saveObject("Players/" + p.getName() + " " + p.getUniqueId(), p);
		}
		if (Main.getInstance().getConfigs().getBoolean("custom-join-message")) {
